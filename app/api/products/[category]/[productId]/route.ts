...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Map the array data to a structured product object
    const productData = {
      id: foundProduct[6], // Product ID is in column 6
      image: foundProduct[0] || "",
      name: foundProduct[1] || "",
      description: foundProduct[3] || "",
      subTitle: foundProduct[2] || "",
      price: foundProduct[4] || "0",
      category: foundInCategory, // The actual category where the product was found
      categoryName: foundCategoryName, // The display name of the category where it was found
      discountPercentage: discountInfo.discountPercentage,
      image2: foundProduct[7],
      image3: foundProduct[8],
      link: foundProduct[9],
      proDetails: nutritionDetails || [], // Add nutrition details here
    };

    console.log(
      `✅ Product data assembled with ${
    ...
    return NextResponse.json({
      product: productData,
      category: foundInCategory, // Return the actual category where the product was found
      categoryName: foundCategoryName, // Return the actual category name
      searchedInAllCategories: category === "all", // Flag to indicate if this was an "all" search
    });
  } catch (error) {
