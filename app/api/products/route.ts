...
export async function GET(request: NextRequest) {
  try {
    ...
    const category = searchParams.get("city") || "all";

    console.log("API: Fetching data for city:", category);
    ...
    if (category === "all") {
      ...
      const [categoryMeta, dynamicAllCategories] = await Promise.all([
        ...
        getAllCategoriesDataById("cities", SPREADSHEET_ID),
      ]);
      ...
      console.log("API: Dynamic all categories", dynamicAllCategories);

      const allSheetIds = categoryMeta.map((cat: any) => cat.id);
      console.log("API: All sheet IDs:", allSheetIds);
    ...
    console.log(
      `API: Successfully fetched ${sheetData.length} items for category "${category}"`
    );
