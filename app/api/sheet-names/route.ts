...
const PRODUCTS_SPREADSHEET_ID = process.env.NEXT_PUBLIC_GOOGLESHEETS_TODAYPICKS_ID!;
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`Fetching sheet names for type: ${type}`);
    ...
    console.log(`Successfully fetched ${sheets.length} ${type} sheets`);
  ...
  try {
    ...
    const results = await Promise.allSettled(
      types.map(async (type: string) => {
        const spreadsheetId = type === 'products' ? PRODUCTS_SPREADSHEET_ID : COMMUNITY_SPREADSHEET_ID;
